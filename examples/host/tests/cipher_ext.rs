...
#[tokio::test]
async fn sm4_cbc_round_trip() {
    let mut f = Fixture::new().await.unwrap();
    let key = vec![0x55u8; 16];
    let iv = vec![0x66u8; 16];
    let pt = b"sm4 cbc test vector".to_vec();
    let ct = f.bindings.openssl_component_cipher()
        .call_encrypt(&mut f.store, ciph::Algorithm::Sm4Cbc,
                      &key, Some(&iv), ciph::PaddingMode::Pkcs7, &pt)
        .await.unwrap().unwrap();
    let got = f.bindings.openssl_component_cipher()
        .call_decrypt(&mut f.store, ciph::Algorithm::Sm4Cbc,
...
#[tokio::test]
async fn sm4_gcm_round_trip() {
    let mut f = Fixture::new().await.unwrap();
    let key = vec![0x77u8; 16];
    let nonce = vec![0x88u8; 12];
    let aad = b"sm4-gcm aad".to_vec();
    let pt = b"sm4-gcm via EVP_CIPHER_fetch".to_vec();
    let sealed = f.bindings.openssl_component_cipher()
        .call_seal(&mut f.store, ciph::Algorithm::Sm4Gcm,
                   &key, &nonce, &aad, &pt, 16).await.unwrap().unwrap();
    let opened = f.bindings.openssl_component_cipher()
        .call_open(&mut f.store, ciph::Algorithm::Sm4Gcm,
                   &key, &nonce, &aad, &sealed.ciphertext, &sealed.tag)
        .await.unwrap().unwrap();
    assert_eq!(pt, opened);
}
